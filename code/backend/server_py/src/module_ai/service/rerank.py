...
引擎方案由 model_config 表驱动(构建统一走 utils.llm.factory 工厂):
    - model_type=rerank + server_type=dashscope/ollama/vllm/openai 的记录即为可选方案
...
class RerankService:
    ...
    async def get_reranker(self, model_id: str | None = None) -> Rerank:
        ...
        if cache_key not in self._rerankers:
            # 构建失败(ValueError: 方案不支持/分数范围非法)统一转 BusinessError 保持接口错误语义
            try:
                engine = build_rerank_model(config)
            except ValueError as e:
                raise BusinessError(str(e))
            self._rerankers = {cache_key: engine}
