...
class ParagraphParse:
    ...
    def run(self, text_blocks: list):
        ...
        # 确保从上到下有序
        text_blocks.sort(key=lambda a: a["box"][0][1])
        # 处理前后顺序
        self.sortLeftRight(text_blocks)  # 确保从左到右有序
    ...
    def sortLeftRight(self, text_blocks):
        # units.sort(key=lambda a: a[0][1])  # 确保从上到下有序
        para_l, para_top = text_blocks[0]["box"][0]  # 当前段的左上
        para_r, para_bottom = text_blocks[0]["box"][2]  # 当前段的右下
        para_line_h = para_bottom - para_top  # 当前段行高
        # 取 左右相等为一个自然段的主体
        for i in range(1, len(text_blocks)):
            l, top = text_blocks[i]["box"][0]  # 当前块左上
            r, bottom = text_blocks[i]["box"][2]  # 当前块右下
            h = bottom - top
            # 大概在同一行
            if (
                abs(para_top - top) <= para_line_h * 0.5
                and abs(para_bottom - bottom) <= para_line_h * 0.5
                and
                # 1在2右侧
                para_l > l
                and para_r > r
    ...
    def _parse(self, units):
        ...
        para_line_w = para_r - para_l  # 当前段行宽
        para_line_h = para_bottom - para_top  # 当前段行高
        ...
        for i in range(1, len(units)):
            ...
            # 检测是否同一段
            if (  # 左右边缘都相等
                abs(para_l - l) <= para_line_h * 0.8
                and abs(para_r - r) <= para_line_h * 0.8
                # abs(para_l - l) <= para_line_w * 0.1
                # and abs(para_r - r) <= para_line_w * 0.1
                # 行间距不大
        ...
        for i1 in reversed(range(len(paras))):
            ...
            if len(para) == 1:
                ...
                if i1 > 0:
                    ...
                    # 检查行间距
                    if (
                        paras_line_space[i1 - 2] != None
                        and top - up_bottom > paras_line_space[i1 - 2] + up_h * 0.5
                    ):
                        up_flag = False
                # 下段开头条件：右对齐/单行超出，左缩进
                if i1 < len(paras) - 1:
