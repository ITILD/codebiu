...
from module_office.utils.document_chunk.chunk_strategy.utils.nlp import count_tokens
import logging
from module_rag.service.user_model import UserModelService
from module_ai.utils.llm.do.llm_type import ModelType
from module_office.utils.document_chunk.chunk_strategy.utils.md_parser_utils import (
    extract_table_block,
    get_title_path,
    split_text_by_length_and_newline,
)
from module_office.utils.document_chunk.chunk_strategy.utils.table_utils import html_table_to_key_value
...
def chunk_markdown(
    markdown_content: str, parser_config: dict[str, Any] | None = None, embed_fn: Any | None = None,user_id: str | None = None
) -> list[str]:
    """
    语义化切分 Markdown 内容。
    ...
    if embed_fn is None:
        try:
            ...
            model_result = user_model_service.get_llm_by_user_id(user_id, False, ModelType.EMBEDDINGS)
            # embed_model = user_model_service.get_llm_by_user_id(user_id,False,ModelType.EMBEDDINGS)
            # 2.如果是协程，强制同步执行它
            if inspect.iscoroutine(model_result):
                logger.info("检测到模型加载返回了协程，正在使用 asyncio.run 同步执行...")
                embed_model = asyncio.run(model_result)
            else:
                embed_model = model_result
            
            # 3. LangChain 用 embed_documents，HuggingFace 用 encode
            if hasattr(embed_model, "embed_documents"):
                embed_fn = embed_model.embed_documents
                logger.info(f"成功加载 Embedding 模型 (使用 embed_documents)")
            elif hasattr(embed_model, "encode"):
                embed_fn = embed_model.encode
                logger.info(f"成功加载 Embedding 模型 (使用 encode)")
            else:
                raise AttributeError(f"模型对象 {type(embed_model)} 缺少 embed_documents 或 encode 方法")
