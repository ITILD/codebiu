...
class ChunkStrategyRecommendation(BaseModel):
    """大模型推荐的分块策略"""
    strategy: ChunkStrategyEnum = Field(default=ChunkStrategyEnum.GENERAL,description="""
    分块策略对应规则如下：
    - `general`: 通用分块：按分隔符和长度切分，适合大多数普通文档、报告、新闻。
    - `tables`: 表格分块：专门处理包含表格的文档，适合数据报告、统计资料等。
    """)
    
    reason: str = Field(default="默认分块策略",description="简洁推荐该策略的简短理由，例如：'文档包含大量问答对，适合QA策略'")
