...
from module_websearch.utils.engines import ENGINE_CLASSES
...
class WebSearchService:
    ...
    def get_engine(self, engine_name: str | None) -> SearchEngine:
        """
        按名称获取引擎(为空返回默认引擎)
        :param engine_name: 引擎标识(duckduckgo/bing)
        :return: 引擎实例
    ...
    async def search(
        self, query: str, engine_name: str | None = None, limit: int | None = None
    ) -> SearchResponse:
        """
        执行网页搜索
        :param query: 查询词
        :param engine_name: 引擎标识(为空使用默认引擎 duckduckgo)
