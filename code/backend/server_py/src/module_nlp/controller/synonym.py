...
from module_nlp.dependencies.synonym import (
    get_synonym_group_service,
    get_synonym_service
)
...
@router.get(
    "/groups/scroll", 
    summary="同义词组滚动加载", 
    response_model=InfiniteScrollResponse
)
...
@router.get(
    "/groups/{group_id}/synonyms", 
    summary="获取同义词组的所有同义词", 
    response_model=list[str]
)
...
@router.post(
    "/synonyms/search/batch", response_model=list[SynonymBatchSearchResult], summary="同义词批量查找")
