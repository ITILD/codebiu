<template>
  <div flex flex-col h-app w-full bg-note-paper overflow-hidden>
    ...
    >
      ...
      <!-- 次级管理入口(小字低调, 不抢历史列表) -->
      <div class="note-edge-t" px-3 py-2.5 space-y-0.5>
        <RouterLink
          to="/agent/manage"
          ...
          <div class="min-h-[60%]" flex flex-col items-center justify-center py-8 text-center>
            ...
            <div grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-8 w-full max-w-lg>
              <button
                ...
                flex items-center gap-3 p-3.5 rounded-xl text-left transition-all
                :class="selectedAgentId === a.id
                  ? 'bg-note-tint shadow-note'
                  : 'bg-note-card hover:shadow-note hover:-translate-y-0.5'"
...
import { Plus, Search, Delete, Fold, Expand, MagicStick, ChatDotRound } from '@element-plus/icons-vue'
...
// 选中会话并加载历史消息(桌面保持侧栏展开, 仅移动端抽屉自动收起; 恢复关联智能体与过程区块)
